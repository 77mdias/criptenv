...
@click.pass_context
def ci_login(ctx, token: str, project: Optional[str]):
...
@click.pass_context
def ci_secrets(ctx, environment: str):
...
@click.option("--provider", help="Sync to provider after push (e.g., vercel, netlify)")
@click.option("--dry-run", is_flag=True, help="Show what would be deployed without actually deploying")
@click.pass_context
def ci_deploy(ctx, environment: str, provider: Optional[str], dry_run: bool):
    ...
    async def _do_deploy():
        ...
        # Implementation would push local secrets to API
...
@click.pass_context
def ci_tokens_list(ctx, include_revoked: bool):
...
@click.pass_context
def ci_tokens_create(ctx, name: str, scopes: str, environment_scope: Optional[str], expires_days: Optional[int]):
...
@click.pass_context
def ci_tokens_revoke(ctx, token_id: str, force: bool):
    """Revoke a CI token.
