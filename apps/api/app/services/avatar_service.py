...
def _validate_image(file: UploadFile) -> tuple[bytes, str]:
    ...
    if content_type not in VALID_IMAGE_TYPES:
        raise HTTPException(
            ...
            detail=f"Invalid image type: {content_type}. Only PNG and JPG are allowed."
        )
...
def _get_storage_config() -> tuple[str, str, str]:
    ...
    return settings.SUPABASE_URL.rstrip("/"), settings.SUPABASE_SERVICE_KEY, settings.SUPABASE_AVATAR_BUCKET or "avatars"
...
class AvatarService:
    ...
    async def upload_avatar(self, user_id: uuid.UUID, file: UploadFile) -> str:
        ...
        """
        content, ext = _validate_image(file)
        ...
        try:
            ...
            raise HTTPException(
                ...
                detail=f"Failed to upload avatar to storage: {str(exc)}"
            ) from exc
    ...
    async def delete_avatar(self, user_id: uuid.UUID) -> None:
        ...
        try:
            ...
            # 200/404 are both acceptable
            if response.status_code not in (200, 204, 404):
                # Non-fatal: log but don't raise to user
                pass
