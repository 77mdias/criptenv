...
class EmailService:
    ...
    Provides professionally designed HTML emails with clean light-theme branding
    ...
    def _base_template(title: str, content_html: str, preheader: str = "") -> str:
        """Wrap content in a professional light-themed email layout."""
        logo_url = settings.FRONTEND_URL.rstrip("/") + "/images/logocriptenv.png"
  ...
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="background-color:#f3f4f6;">
    <tr>
      <td align="center" style="padding: 40px 16px 24px;">
        <table role="presentation" class="container" width="600" cellspacing="0" cellpadding="0" border="0" style="max-width:600px;width:100%;background-color:#ffffff;border-radius:12px;overflow:hidden;border:1px solid #e5e7eb;">
          <!-- Header -->
          <tr>
          ...
          <!-- Footer -->
          <tr>
            <td style="padding:24px 32px 32px;border-top:1px solid #e5e7eb;text-align:center;">
    ...
    def _url_box(url: str) -> str:
        """Generate a styled URL display box."""
        return f"""
    ...
    def send_password_reset(self, to: str, reset_url: str, expires_minutes: int = 60) -> Optional[dict]:
        ...
        <p style="margin:0 0 16px;font-size:15px;color:#374151;line-height:1.6;">
          We received a request to reset the password for your CriptEnv account (<strong style="color:#111827;">{to}</strong>).
        ...
        <p style="margin:16px 0 0;font-size:13px;color:#6b7280;">
          This link expires in <strong style="color:#374151;">{expires_text}</strong> and can only be used once.
        </p>
...
"""
    ...
    def send_2fa_enabled(self, to: str) -> Optional[dict]:
        ...
        content = """
        <h1 style="margin:0 0 16px;font-size:22px;font-weight:700;color:#111827;">Two-Factor Authentication Enabled</h1>
        <p style="margin:0 0 16px;font-size:15px;color:#374151;line-height:1.6;">
...
"""
    ...
    def send_account_deleted(self, to: str) -> Optional[dict]:
        ...
        content = """
        <h1 style="margin:0 0 16px;font-size:22px;font-weight:700;color:#111827;">Account Deleted</h1>
        <p style="margin:0 0 16px;font-size:15px;color:#374151;line-height:1.6;">
...
"""
    ...
    def send_email_verification(self, to: str, verification_url: str, expires_hours: int = 24) -> Optional[dict]:
        ...
        content = f"""
        <h1 style="margin:0 0 16px;font-size:22px;font-weight:700;color:#111827;">Welcome to CriptEnv</h1>
        <p style="margin:0 0 16px;font-size:15px;color:#374151;line-height:1.6;">
        ...
        <p style="margin:0 0 16px;font-size:15px;color:#374151;line-height:1.6;">
          ...
          (<strong style="color:#111827;">{to}</strong>). This helps us protect your account and
        ...
        <p style="margin:16px 0 0;font-size:13px;color:#6b7280;">
          This verification link expires in <strong style="color:#374151;">{expires_text}</strong>.
        </p>
        ...
        <p style="margin:24px 0 0;font-size:14px;color:#6b7280;line-height:1.6;border-top:1px solid #e5e7eb;padding-top:16px;">
          <strong style="color:#374151;">What is CriptEnv?</strong><br>
...
"""
    ...
    def send_welcome(self, to: str, name: str = "") -> Optional[dict]:
        ...
        content = f"""
        <h1 style="margin:0 0 16px;font-size:22px;font-weight:700;color:#111827;">You're all set, {name or "welcome aboard"}!</h1>
        <p style="margin:0 0 16px;font-size:15px;color:#374151;line-height:1.6;">
        ...
        <ul style="margin:0 0 20px;padding-left:20px;color:#374151;font-size:15px;line-height:1.7;">
          <li>Create your first <strong style="color:#111827;">project</strong> to organize your secrets</li>
          <li>Set up <strong style="color:#111827;">environments</strong> (Production, Staging, Development)</li>
          <li>Push secrets via our <strong style="color:#111827;">CLI</strong> or the web dashboard</li>
          <li>Invite team members and control access with <strong style="color:#111827;">role-based permissions</strong></li>
          <li>Enable <strong style="color:#111827;">Two-Factor Authentication</strong> for extra security</li>
        </ul>
...
"""
    ...
    def send_project_invite(
        ...
        inviter_text = f" <strong style=\"color:#111827;\">{invited_by_name}</strong> has invited you" if invited_by_name else " You've been invited"

        content = f"""
        <h1 style="margin:0 0 16px;font-size:22px;font-weight:700;color:#111827;">Project Invitation</h1>
        <p style="margin:0 0 16px;font-size:15px;color:#374151;line-height:1.6;">
          {inviter_text} to join the project <strong style="color:#111827;">{project_name}</strong> on CriptEnv
          with <strong style="color:#111827;">{role}</strong> access.
        </p>
        <p style="margin:0 0 16px;font-size:15px;color:#374151;line-height:1.6;">
        ...
        <p style="margin:16px 0 0;font-size:13px;color:#6b7280;">
          This invitation expires in <strong style="color:#374151;">{expires_days} days</strong>.
        </p>
...
"""
    ...
    def send_email_changed(self, to: str, old_email: str) -> Optional[dict]:
        ...
        content = f"""
        <h1 style="margin:0 0 16px;font-size:22px;font-weight:700;color:#111827;">Email Address Updated</h1>
        <p style="margin:0 0 16px;font-size:15px;color:#374151;line-height:1.6;">
          The email address for your CriptEnv account has been successfully updated.
        </p>
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="margin:16px 0;background-color:#f9fafb;border-radius:8px;border:1px solid #e5e7eb;">
          <tr>
          ...
          <tr>
            <td style="padding:0 24px 20px;">
              ...
              <p style="margin:0;font-size:15px;color:#10b981;font-weight:500;">{to}</p>
            </td>
          </tr>
        </table>
...
"""
    ...
    def send_email_changed_alert_to_old(self, to: str, new_email: str) -> Optional[dict]:
        ...
        content = f"""
        <h1 style="margin:0 0 16px;font-size:22px;font-weight:700;color:#111827;">Security Alert: Email Address Changed</h1>
        <p style="margin:0 0 16px;font-size:15px;color:#374151;line-height:1.6;">
        ...
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="margin:16px 0;background-color:#f9fafb;border-radius:8px;border:1px solid #e5e7eb;">
          <tr>
            <td style="padding:20px 24px;">
              ...
              <p style="margin:0;font-size:15px;color:#111827;font-weight:500;">{new_email}</p>
            </td>
          </tr>
        </table>
        <p style="margin:16px 0 0;font-size:15px;color:#374151;line-height:1.6;">
